...
public class Fabrica {
	...
	Album crearAlbumTradicional() {
		Integer codigoSerie;
		String premio;
		String []paisesParticipantes;
		return null;
	}

	List<Figurita> generarSobre(int cantFigus) {
		ArrayList<Figurita> paquete = new ArrayList<Figurita>(4);
	...
	private String obtenerPais() {
		 int paisObtenido = paisesParticipantes.length;
			
		 int randomNumber = (int) (Math.random()*32+1);
		 
		 String paisElegido = null;
		 
		for(int i=0;i<paisesParticipantes.length;i++) {
			if(randomNumber==i) {
				paisElegido= paisesParticipantes[i];
			}
		}
		return paisElegido;
